...
class Inv_UomController extends Controller
{
    ...
    public function store(InvUomRequest $request)
    {
        ...
        try {
            ...
            if ($checkExists == null) {
                ...
                Inv_uom::create($data);
                return redirect()->route('admin.uoms.index')->with(['success' => 'لقد تم اضافة الوحدة بنجاح']);
            } else {
    ...
    public function update($id, InvUomUpdateRequest $request)
    {
        try {
            ...
            return redirect()->route('admin.uoms.index')->with(['success' => 'لقد تم تحديث الوحدة بنجاح']);
        } catch (\Exception $ex) {
